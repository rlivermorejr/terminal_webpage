...
export default function Term() {
	...
	const [prompt, setPrompt] = React.useState("you@/terminal:~$ ");
	const [home, sethome] = React.useState("term");
				...
				cd: {
					...
					fn: (...args) => {
						if (args.length === 1 && args[0] === "..") {
							if (prompt === "you@/terminal:~$ ") {
								return "cannot go up";
							} else {
								setPrompt(prompt.substring(0, prompt.lastIndexOf("/")) + ":~$ ");
								sethome(
									prompt.substring(prompt.lastIndexOf("/", prompt.lastIndexOf("/") - 1) + 1, prompt.lastIndexOf("/")),
								);
					...
					fn: () => {
						return `
                            ${Object.keys(owrs)
															.map(
																(cmd) =>
																	`${cmd}${" ".repeat(12 - cmd.length)} | ${owrs[cmd].description}${" ".repeat(
                            ...
                            ${Object.keys(cmds)
															.map(
																(cmd) =>
																	`${cmd}${" ".repeat(12 - cmd.length)} | ${cmds[cmd].description}${" ".repeat(
